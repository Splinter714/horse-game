...
export const WithInteractables = (Base) => class extends Base {
  buildInteractables() {
    ...
    this.interactables = [gate, house, shop, barn, gardenPlant, tractor, trough, catFoodBowl, catWaterBowl, bunnyFoodBowl, bunnyWaterBowl, seedFeeder, nectarFeeder, beehive, sources, nests, farmStand, standWoolDump, spinningWheel, kitchenCounter, compostBin, trashCan, gardenHarvest];
    // Split by input: gate/house/shop/barn/garden-plant/tractor are bare-hand "interact"
    // targets (tap/click/E); the rest require a carried tool/carrier and are triggered by
    // Use (the on-screen button / F / controller). See useActiveTool + handleTap.
    this.interactWorld = [gate, house, shop, barn, gardenPlant, tractor];
