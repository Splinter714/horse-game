...
| Bird ecosystem | `paddock/birdEcosystem.js` (`WithBirdEcosystem`) | the fixed bird-ecosystem props + their object-anchored bird visits. Bird bath (#219, ambient splashing), refillable seed feeder (#240, `fillSeedFeeder`/`_setSeedFeederLevel`/`drainSeedFeeder`; birds visit only while stocked), and later the hummingbird feeder (#226) + beehive (#239). `buildBirdEcosystem` places sprites/`props.*` + collects footprints into `this.birdEcosystemObstacles` (spread by `world.js`); the visit beats (`_spawnBirdBathVisit`/`_spawnFeederVisit`) reuse `wildlife.js`'s shared `_pickBird`/`_birdTakeOff`/`_wildCritters`/`WILD_SCALE`, kicked off from `buildWildlife` via `startBirdEcosystemVisits`. Pure fill/drain math in `data/feeder.js` |
