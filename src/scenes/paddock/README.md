...
| Breeding & foals | `paddock/breeding.js` (`WithBreeding`) | player-initiated pairing → gestation → birth (#15): `toggleBreedSelection`/`beginBreeding` (pair two horses from the info panel), `updateBreeding` (tick gestations, moves born-ready ones to the holding queue — #299), `flushReadyBirths`/`_announceOvernightBirths` (reveal held births at wake-up, #299: a completed gestation is HELD, not birthed live, until `EVENTS.SLEEP_DONE` flushes it as a "while you were sleeping" surprise), `_birthFoal` (new foal joins `allHorses`, spawns via `spawnHorse`, opens the customizer parent-seeded), `growUpFoal`/`setStayBaby` (the "stay a baby forever" toggle gates growth on the day roll), `spawnSavedFoals` (restore bred horses on boot). Pure timing/seed/roster-key logic + in-flight gestation + ready-to-birth persistence live in `data/breeding.js` + `save.js` `load/saveGestations`, `load/saveReadyBirths` |
