...
| Fox taming | `paddock/fox.js` (`WithFox`) | `onFoodPlaced`/`_lureWildFox`/`_commitFox` (#266): dropping a **fox-food** pile summons a wild fox that trots over to eat; repeated feeding (`feedWildFox` counter, persisted via `save.js` `loadFoxTaming`/`saveFoxTaming`) tames it into the roster (cap 1) via the generic `_spawnWorldIndividual`. Pure taming counter + cap live in `data/species/fox`; the tamed fox is an ordinary grazer (`seekFoxFood`/`seekFoxWater`, `behaviors.js`). Own module so the parallel owls agent doesn't touch `wildlife.js` |
