...
| Chicken coop | `paddock/chickenCoop.js` (`WithChickenCoop`) | in-world coop CUTAWAY (#53), mirroring the barn's walk-in pattern (#35): `buildChickenCoop` places the interior (floor/nestboxes/roost bar, drawn under the flock) + a front-façade overlay that `updateCoopCutaway` fades near-transparent when the player is near/inside, and `_coopRoostSpotFor` gives each roosting bird a stable, VISIBLE spot inside instead of the old vanish-on-roost. Purely visual — reuses the existing roost/leave-coop flow in `dayNight.js` (`chickenRoost`/`chickenLeaveCoop`), no new AI |
