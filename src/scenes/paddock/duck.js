// Duck taming (#275, second duck #409) — the scene-coupled half of "leave food out for
// the wild duck and it warms up to you until it moves in." Self-contained in its OWN
// mixin (mirrors paddock/fox.js, plus the bunny's untaken-variant picking for the
// second individual): it owns the wild-duck sprite, the `onDuckFoodPlaced` taming
// hook, and the commit-to-roster spawn.
//
...
//      code here.
//
// Two ducks can ever join — one male, one female (DUCK_CAP = DUCK_VARIANTS.length,
// #409). While there's still an open slot, `onDuckFoodPlaced` picks the next untaken
// sex (`nextDuck`, mirrors the bunny's `nextBunny` untaken-coat pick) the FIRST time a
// pile is dropped after the previous duck committed, and lures that wild duck in — same
// taming counter, reset to 0 so the second duck's befriending starts fresh rather than
// instantly completing on its first feed. Once both slots are filled, the dropped pile
// just feeds the roster ducks through the normal grazing AI.
//
// A returning player's already-tamed duck(s) are restored from the persisted roster on
...
import { DUCK_KEYS, DUCK_CAP, feedWildDuck, nextDuck } from '../../data/species/duck/index.js';
...
export const WithDuck = (Base) => class extends Base {
  // Called from create(): set up the duck animations (for EVERY possible duck slot —
  // whichever sex waddles in first needs its frames ready) + restore the persisted
  // taming count so a returning player's warm-up carries over. No wild duck is spawned
  // up front — one only waddles in when the player drops duck food (onFoodPlaced).
  buildDuck() {
    ...
    // The duck's idle/walk/eat animations, keyed per slot. spawnAnimal would create
    ...
    DUCK_KEYS.forEach((key) => {
      if (this.anims.exists(`idle_${key}`)) return;
  ...
  // Is the duck roster already full? (Both sexes tamed — no more wild-phase summoning.)
  _duckRosterFull() {
  ...
  // the roster is already full, do nothing — the roster ducks eat the pile via the
  ...
  onDuckFoodPlaced(content, x, y) {
    ...
    if (this._duckRosterFull()) return; // already have both ducks — normal grazing takes over
  ...
  // waddling in rather than popping into being on the pile. Picks the next untaken sex
  // (nextDuck, among the ducks already in the roster) and remembers it on the wild-duck
  // record so the commit spawns the SAME sex/key it's been waddling around as — the
  // look is continuous through the commit, same as the single-duck case used to be.
  _spawnWildDuck(x, y) {
    const all = this.registry.get('allDucks') ?? {};
    const takenSexes = Object.values(all).map((d) => d.sex);
    const variant = nextDuck(takenSexes);
    if (!variant) return; // defensive — roster already full
    ...
    const sprite = this.add.sprite(fromX, fromY, `${key}_idle_0`)
      .setOrigin(0.5, 1).setScale(DUCK_SCALE).setDepth(fromY).play(`idle_${key}`);
    const shadow = this.add.image(fromX, fromY, 'shadow')
      .setScale(S * (SPECIES.duck.spawn.shadowScale ?? 0.24)).setDepth(fromY - 1);
    this._wildDuck = { sprite, shadow, tween: null, variant };
  }
  ...
  _duckWaddleTo(c, tx, ty, onArrive) {
    if (!c.sprite?.active) return;
    const key = c.variant?.key ?? DUCK_KEYS[0];
    const sprite = c.sprite;
    sprite.setFlipX(tx < sprite.x);
    sprite.play(`walk_${key}`, true);
  ...
  _feedWildDuck(c, x, y) {
    ...
    this.time.delayedCall(1600, () => {
      if (step.tamed) {
        ...
        c.sprite.play(`idle_${key}`, true);
        this.time.delayedCall(Phaser.Math.Between(600, 1400), () => this._duckWaddleOff(c));
      }
    });
  }

  // Commit the tamed duck: remove the wild-duck sprite and spawn a real roster Duck (of
  // the sex it's been waddling around as) at the same spot via the generic
  // `_spawnWorldIndividual` (the exact wiring buildAnimals uses), so from now on it's an
  // ordinary cared-for grazer that also swims. Adds it to the persisted `allDucks`
  // roster and saves immediately so it survives a reload even before the autosave tick
  // — directly mirrors `_commitFox`. Resets the taming counter so a still-open second
  // slot starts its own befriending from scratch rather than instantly completing.
  _commitDuck(c, x, y) {
    const at = { x: c?.sprite?.x ?? x, y: c?.sprite?.y ?? y };
    const variant = c?.variant;
    this._despawnWildDuck(c);
    if (!variant || this._duckRosterFull()) return; // defensive — already committed/full

    const all = this.registry.get('allDucks') ?? {};
    const model = new Duck({ coat: variant.coat, sex: variant.sex, name: variant.name });
    all[variant.key] = model;
    this.registry.set('allDucks', all);

    this._duckTameCount = 0;
    saveDuckTaming(0);

    const a = this._spawnWorldIndividual(SPECIES.duck, variant.key, model, { x: at.x, y: at.y });
