...
| Bird ecosystem | `paddock/birdEcosystem.js` (`WithBirdEcosystem`) | the fixed bird-ecosystem props + their object-anchored flying-wildlife visits. Bird bath (#219, ambient splashing), refillable seed feeder (#240, `fillSeedFeeder`/`_setSeedFeederLevel`/`drainSeedFeeder`; songbirds visit only while stocked), hummingbirds + nectar feeder (#226, `fillNectarFeeder`/`drainNectarFeeder`; hover-and-dart `_spawnHummingbird`/`_hummerDartTo`/`_hummerHover` drawn to the STOCKED nectar feeder AND the flowers, `_hummerTargets`), and later the beehive (#239). `buildBirdEcosystem` places sprites/`props.*` + collects footprints into `this.birdEcosystemObstacles` (spread by `world.js`); the visit beats reuse `wildlife.js`'s shared `_pickBird`/`_birdTakeOff`/`_despawnCritter`/`_wildCritters`/`WILD_SCALE`, kicked off from `buildWildlife` via `startBirdEcosystemVisits`. Pure feeder fill/drain math in `data/feeder.js`; nectar is its own bucket resource gathered at the nectar station |
